...
有些情况普通数组没法解决：n×mn×m 的矩阵，1≤n,m≤1061≤n,m≤106 且 n×m≤106n×m≤106
...
- 每次插入后进行快速排序：k⋅nlognk⋅nlog⁡n
- 使用优先队列维护：k⋅lognk⋅log⁡n
...
- 若 a⋆ba⋆b，则比较器函数应当返回 `true`
- 若 a⋆̸ba⋆̸b，则比较器函数应当返回 `false`
...
- `lower_bound()`: 寻找 ≥x≥x 的第一个元素的位置
- `upper_bound()`: 寻找 >x>x 的第一个元素的位置

怎么找 ≤x≤x / <x<x 的第一个元素呢？

- \>x>x 的第一个元素的前一个元素（如果有）便是 ≤x≤x 的第一个元素
- ≥x≥x 的第一个元素的前一个元素（如果有）便是 <x<x 的第一个元素
...
| f(x,y)=xyf(x,y)=xy   | `pow(2, 3)`  |
| f(x)=x−−√f(x)=x      | `sqrt(2)`    |
| f(x)=⌈x⌉f(x)=⌈x⌉     | `ceil(2.1)`  |
| f(x)=⌊x⌋f(x)=⌊x⌋     | `floor(2.1)` |
| f(x)=⟨x⟩f(x)=⟨x⟩     | `rount(2.1)` |
...
- ⌈ab⌉⌈ab⌉
  - 别用：`ceil(1.0 * a / b)`
  - 要用：`(a + b - 1) / b` （⌈ab⌉=⌊a+b−1b⌋⌈ab⌉=⌊a+b−1b⌋）
- ⌊a−−√⌋⌊a⌋
  - 别用：`(int) sqrt(a)`
